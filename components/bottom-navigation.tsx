...
export function BottomNavigation() {
  ...
  const navLinkClass = (path: string) =>
    cn(
      ...
      isActive(path) ? "text-brand font-medium" : "text-muted-foreground",
    )
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 border-t bg-background/80 backdrop-blur-lg z-50">
      <div className="flex justify-around items-center h-16 relative">
        <Link href="/dashboard" className={navLinkClass("/dashboard")}>
          <div className="relative">
            ...
            {pendingEmailIngestionCount > 0 && (
              <span className="absolute -top-1 -right-1 h-2 w-2 rounded-full bg-brand animate-pulse" />
            )}
          </div>
          <span>Home</span>
          {isActive("/dashboard") && (
        ...
        <Link
          href="/add-item"
          className="absolute -top-7 left-1/2 -translate-x-1/2 flex items-center justify-center w-14 h-14 rounded-full bg-brand text-white shadow-lg active:scale-90 transition-transform duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2"
        >
        ...
        <Link href="/shopping-list" className={navLinkClass("/shopping-list")}>
          <div className="relative">
            ...
            {incompleteCount > 0 && (
              <span className="absolute -top-1.5 -right-1.5 h-4 w-4 rounded-full bg-destructive text-white text-[10px] font-bold flex items-center justify-center leading-none">
