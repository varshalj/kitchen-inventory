...
- **Rating chip doesn't appear after voice-driven `mark_as_consumed`.** Observed in real use: when the user taps Consume in the UI, the dashboard's onConsume handler calls `queueForReview()` from `ReviewContext` and the rating chip surfaces 5.5s later (post-undo window). When voice triggers the same action via the MCP `mark_as_consumed` tool, the write succeeds but `queueForReview` is never called — the dashboard component doesn't know voice did the consume, so its handler-side review-queueing never runs. The fix is a new RTVI server-message: after a successful confirm=true `mark_as_consumed`, the voice agent emits `{type: "queue_review", data: {item: <full inventory row>, type: "consumed"}}`. Browser handler in `voice-mic-button.tsx` calls `useReview().queueForReview(item, type)`. Same machinery as the `toast` and `navigate_to` server-messages — just a new dispatch case + a tweak to the `_write_with_toast` wrapper to include the consumed item's row in the response so the message has a full `InventoryItem` to pass. Same applies to a future `mark_as_wasted` voice tool. Estimated 1-2 hours. Worth doing before Stage 4 polish since it leaves a real UX gap: rating data informs all future recommendations and voice users currently can't contribute any.

- **User-facing privacy disclosure for voice transcripts (NOT shipped yet — flagged in ADR 010 design, never built).** Voice sessions write transcripts to the `voice_session_logs` table when `feature_grants.voice_logs_enabled = true` (admin-controlled per user, opt-in by default). ADR 010's Decision section described an "About voice" link on the widget opening a short modal that explains: audio goes to Modal, transcripts to Sarvam, OpenAI generates responses, conversations may be logged for debugging if logging is enabled. **This modal does not exist.** The `/privacy` page does not mention voice at all. The profile page has no voice-logging toggle. For household-only use this is acceptable (both household members know logging happens — it's the developer's debugging). **Before inviting any user outside the household this MUST ship**: (1) an "About voice" tooltip/modal on the mic button surfacing the data flow, (2) a sentence on `/privacy` covering voice specifically, (3) a user-controlled toggle (paired with the `voice_opt_out` user-self-disable backlog item — same `user_settings` row could carry both `voice_opt_out` and `voice_logs_opt_out`). The admin-controlled `feature_grants.voice_logs_enabled` is the right *server-side* default (opt-in by deliberate grant), but it's not a *disclosure* — users can't see what's enabled for them. Estimated half-day. Trigger to pick up: first invite-out-of-household.
